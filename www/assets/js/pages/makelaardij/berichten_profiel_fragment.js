...
function create_or_update() {
	...
	if ( isset( get_global_var( global_vars.USER_ID ) ) && get_global_var( global_vars.USER_ID ) != 0 ) {
        ...
        if(get_global_var( "registered" )=="true")
        ...
        if(get_global_var( "registered" )=="true")
          ...
          main_view.router.loadPage( 'pages/'+app_name.toLowerCase()+'/registration.html' );
...
function berichten_profiel_fragment() {
    ...
    if ( !isset( get_user_id() ) || get_user_id() == 0 || get_user_id() == '0' ) {
        ...
        fetch_record( db_tables.APP_SETTINGS_TABLE, { }, '', function ( data ) {
            ...
            console.log('pasok: fetch_record');
	...
	if ( get_device_platform() == "android" ) {
	    ...
	    $( document )
          ...
          window.plugins.OneSignal.promptForPushNotificationsWithUserResponse(function(value) {
            console.log("User accepted notifications: " + value);
            ...
            if(value){
              ...
              $( '#nextone' ).toggle();
              $( '.bottom-fixed.next.hide' ).removeClass( 'hide' );
              $( '.bottom-fixed.next.hide' ).addClass( 'show' );
            }else{
              //alert(value);
    ...
    fetch_record( db_tables.TARGETGROUP_TABLE, { }, 'ModifiedAt', function ( data ) {
        ...
        populate_select_options(
            ...
            get_pre_selected_options( get_global_var( global_vars.USER_DATA ), 'targetGroup', 'leeg' ),
            false
        );
        ready++;
    } );
...
// latest added code for iPad
$('#nextone').click(function(){
    console.log('---clicked---');
    $('.berichten-profiel-fragment').show();
    $('.popup').hide();
});
