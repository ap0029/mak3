...
- (void)openInInAppBrowser:(NSURL*)url withOptions:(NSString*)options
{
    ...
    // don't change order
    if (browserOptions.fj_navigationbar)
        [self.inAppBrowserViewController showNavigationBar:browserOptions];
    // FVV
...
- (void)show:(CDVInvokedUrlCommand*)command
{
    ...
    // FVV
    if ([self.inAppBrowserViewController getBrowserOptions].fj_navigationbar)
...
// FVV
- (CDVInAppBrowserOptions *)getBrowserOptions {
...
// FVV
- (void)showNavigationBar:(CDVInAppBrowserOptions *)browserOptions
{
    UIView * navbar = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, TOOLBAR_HEIGHT + STATUSBAR_HEIGHT)];
    navbar.backgroundColor = [CDVInAppBrowserViewController _colorwithHexString:browserOptions.fj_barcolor];
    ...
    // label and back arrow have to be centered vertically over the full height of 64 px
    CGRect f = navbar.frame;
    f.origin.x = 40;
    f.origin.y = 0;
    f.size.width -= (2 * 40);
    UILabel * label = [[UILabel alloc] initWithFrame:f];
    label.text = browserOptions.fj_title;
    label.textAlignment = NSTextAlignmentCenter;
    label.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    label.backgroundColor = [UIColor clearColor];
    label.textColor = [CDVInAppBrowserViewController _colorwithHexString:browserOptions.fj_titlecolor];
    label.font = [UIFont systemFontOfSize:label.font.pointSize + 3];
    
    UIButton * button = [[UIButton alloc] initWithFrame:CGRectMake(10, 10 + ((TOOLBAR_HEIGHT - 28) / 2), 28, 28)];
    button.backgroundColor = [UIColor clearColor];
    UIImage * imageArrow = [UIImage imageNamed:@"icon-arrow-back-128"];
    [button setImage:[CDVInAppBrowserViewController filledImageFrom:imageArrow withColor:[CDVInAppBrowserViewController _colorwithHexString:browserOptions.fj_titlecolor]] forState:UIControlStateNormal];
    ...
    if (self.addressLabel && !browserOptions.location)
        [self.addressLabel removeFromSuperview];
    if (self.toolbar && !browserOptions.toolbar)
        [self.toolbar removeFromSuperview];
    
    CGRect webViewBounds = self.view.bounds;
    // for some reason we have to move the webview STATUSBAR_HEIGHT pixels up and extend the height
   // webViewBounds.origin.y = navbar.frame.size.height - STATUSBAR_HEIGHT;
   // webViewBounds.size.height = [UIScreen mainScreen].bounds.size.height - navbar.frame.size.height + STATUSBAR_HEIGHT;

    webViewBounds.origin.y = navbar.frame.size.height;
    webViewBounds.size.height = [UIScreen mainScreen].bounds.size.height - navbar.frame.size.height;
    if (browserOptions.location)
        webViewBounds.size.height -= self.addressLabel.frame.size.height;
    if (browserOptions.toolbar)
        webViewBounds.size.height -= self.toolbar.frame.size.height;
    [self setWebViewFrame:webViewBounds];
}

// https://github.com/nicklockwood/ColorUtils/blob/master/ColorUtils/ColorUtils.m
+ (UIColor *)_colorwithHexString:(NSString *)hexStr
{
    ...
    switch ([hexStr length]) {
        ...
        case 3: {
            NSString *red = [hexStr substringWithRange:NSMakeRange(0, 1)];
            NSString *green = [hexStr substringWithRange:NSMakeRange(1, 1)];
            NSString *blue = [hexStr substringWithRange:NSMakeRange(2, 1)];
            hexStr = [NSString stringWithFormat:@"%1$@%1$@%2$@%2$@%3$@%3$@ff", red, green, blue];
            break;
        }
        case 6: {
...
- (id)init
{
    if (self = [super init]) {
        ...
        // FVV
        self.fj_navigationbar = NO;
        self.fj_barcolor = @"#ffffff";
        self.fj_title = nil;
        self.fj_titlecolor = @"#000000";
        // FVV
    }
